...
let alum = parseInt(prompt("Ingrese numero de Alumnos: "));
//le pedimos la cantidad de alumnos al usuario
function cantAlumnos(alum){
  if(alum < 0 || alum > 0 || alum == 0){
  return alum;
  }else{
    alum = parseInt(prompt("Ingrese numero de Alumnos: "));
    cantAlumnos(alum);
  }
}

function compVacio(notaAlumno){
  if(notaAlumno < 7 || notaAlumno > 1){
  return notaAlumno;
  }else{
    notaAlumno = parseInt(prompt("Ingrese nota de Alumnos+
                                 : "));
    compVacio(notaAlumno);
  }
}
...
function cantNotas(notaAlumno, alumnos){
